"""Train the transformer baseline end to end and score it, fully offline on synthetic data.
...
split them into train / val / test, train the transformer through the shared Lightning harness, run
the trained detector, and score its detections through the same benchmark scorer the published
leaderboard uses. It trains a tiny model for a few epochs on CPU — a demonstration of the pipeline,
not a tuned run, so the headline numbers are not meaningful.
...
``objects_from_labelled_dataset``, and run ``train_transformer`` on a single GPU
(``accelerator="auto"``) within the pinned compute budget — see
``examples/train_transformer_real.py``. The measured cost and test scores are then recorded on the
model card.
...
# A tiny transformer keeps the synthetic CPU demo fast; the real run uses the ~9.5 M default config.
_DEMO_CONFIG = TransformerConfig(
    d_model=32, nhead=4, num_layers=2, dim_feedforward=64, dropout=0.0, max_len=256
)
...
def main() -> int:
    ...
    print("Training the transformer baseline on a synthetic LEO population (CPU, a few epochs)...")
    ...
    detector = TransformerDetector(bundle)
