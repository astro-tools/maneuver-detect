"""Train the transformer baseline on the real, credentialed dataset and score it.
...
Unlike ``train_transformer.py`` (a synthetic, offline demo), this needs Space-Track credentials and
a GPU. Set ``SPACETRACK_USERNAME`` / ``SPACETRACK_PASSWORD`` and run it from a CUDA box:

    export CUBLAS_WORKSPACE_CONFIG=:4096:8       # for deterministic CUDA matmul
    python examples/train_transformer_real.py
...
from maneuver_detect.models.module import LossName
...
def main() -> int:
    ...
    calibration = fit_calibration_on_val(
        TransformerDetector(bundle), series_by_norad, dataset.labels, split
    )
